...
export async function user(interaction: InteractionParam, args: ArgsParam<typeof InfoCommand>["user"]): Promise<void> {
	...
	try {
		if (member) {
			await interaction.editReply({ embeds: [await userInfo(args, member, blacklist!, interaction)] });
		} else {
			const user = interaction.options.getUser("target");
			console.log(user?.flags?.toArray());
			await interaction.editReply({ embeds: [await userInfo(args, user!, blacklist!, interaction)] });
		}
	} catch (error) {
		console.log(error);
