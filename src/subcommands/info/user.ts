...
import { getLanguage, userInfo } from "#utils/index.js";
...
export async function user(interaction: InteractionParam, args: ArgsParam<typeof InfoCommand>["user"]): Promise<void> {
	...
	try {
		if (member) {
			await interaction.editReply({ embeds: [await userInfo(args, member, blacklist!, locale)] });
		} else {
			const user = interaction.options.getUser("target");
			console.log(user?.flags?.toArray());
			await interaction.editReply({ embeds: [await userInfo(args, user!, blacklist!, locale)] });
		}
	} catch (error) {
		console.log(error);
