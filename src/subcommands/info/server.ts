...
export async function server(interaction: InteractionParam, args: ArgsParam<typeof InfoCommand>): Promise<void> {
	...
	const defaultLanguage = (args.server.hide ?? true) ? undefined : "en-US";
	...
	await interaction.editReply({ embeds: [await serverInfo(args, interaction, role, owner, member, emoji, channels, locale)] });
}
