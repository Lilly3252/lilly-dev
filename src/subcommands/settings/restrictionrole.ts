...
export async function restriction_role(interaction: InteractionParam, args: ArgsParam<typeof SettingCommand>["restriction_roles"]): Promise<void> {
	...
	for (const roleKey of roleKeys) {
		if (roles === roleKey.replace("restrict", "").toLowerCase()) {
			await updateRoleSetting(interaction, guildSettings!, role, roleKey, role?.id ?? null, "command.config.events.channel_set", "command.config.events.channel_removed");
		}
	}

	if (roles === "safe") {
		await updateSafeRoles(interaction, guildSettings!, role?.id ?? "", choice);
	}
}
